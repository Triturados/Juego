	...
	namespace ECS {
		...
		void MainMenu::update() {
			...
			if (presskey) {
				...
				presskeytimer += Time::getInstance()->deltaTime;
				if (presskeytimer > 0.5f && (input->anyKeyPressed() || !input->mousePressed(Input::MouseState::NONE))) {
					presskey = false;
		...
		void MainMenu::settings() {
			SceneManagement::changeScene((int)SceneOrder::Settings, SceneManagement::SceneLoad::UP);
		}
		...
		void MainMenu::advance(int idx, float time) {
			...
			for (int i = 0; i < NumButtons; i++) {
				...
				buttons[i]->setInteractable(std::abs(buttonidx - centerButtonIdx) <= 1);
